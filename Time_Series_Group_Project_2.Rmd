...
fit <- auto.arima(lmarathon, ic="aic")
```


We then check the residuals of the auto ariama and see that the mean appears to be constant at as well as the variance. There does appear to be a few outliers particularily at point 120 or so. WHY?
...
```{r}
acf(fit$residuals, lag=150)
pacf(fit$residuals, lag=150)
```
...
```
P-value does not reject null so we can say that the error terms are correalted.
...
The residuals appear normal enough.
...
We can say that it is normal.


```{r}
library("tseries")    
comb_ts <- cbind(lmarathon, fit) # please make sure the length of both your timeseries
...
#data <- data.frame(Year = c(floor(time(marathon) + .01)), marathon)

#  ggplot(data, aes(Year, marathon)) + geom_vline(xintercept = data$marathon[3])

# plot(data$Year, Month = c(cycle(z)), z)
